...
class SfMDataset(Dataset):
    ...
    def get_file_paths(data_path: str):
        ...
        rgb =
        #dense = [Path(file).stem for file in os.listdir(img_dir) if Path(file).suffix == img_extension]
        #sparse = [Path(file).stem[:-len(gt_name_expansion)] for file in os.listdir(gt_dir) if
                      # Path(file).suffix == gt_extension]
